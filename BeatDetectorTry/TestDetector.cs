...
namespace BeatDetectorTry
{
    public class TestDetector
    {
        ...
        public void Update(float[] spectrum)
        {
            ...
            for (int i = 0; i < nBand; i++)
            {
                float specVal = (float)(((Math.Sqrt((double)averages[i])) * 2) * 1);//(float)System.Math.Max(-100.0f, 20.0f * (float)System.Math.Log10(averages[i]) + 160); // dB value of this band
                //specVal *= 0.025f;
